...
function isOrdinal(value: string): boolean {
  ...
  return (
    /^\d{1,3}$/.test(trimmed) ||
    isRomanNumeral(trimmed) ||
    isNumberWord(trimmed)
  );
}
...
function rejectHeadingText(text: string): boolean {
  ...
  if (
    /^(?:contents|table of contents|illustrations?|title page|copyright|preface|introduction)$/i.test(
      text,
    )
  ) {
    return true;
  }
  if (/^\[?(?:illustration|transcriber'?s note|note)\]?$/i.test(text))
    return true;
...
function headingConfidence(block: HtmlBlock): number {
  const text =
    embeddedStructuralHeading(block.text) ?? normalizeText(block.text);
  ...
  if (/^chapter(?:\s+(?:the\s+)?)?[\divxlcdm]+(?:[.)])?(?:\s+.*)?$/i.test(text))
    return 100;
  if (/^chapter(?:\s+(?:the\s+)?)?[a-z-]+(?:[.)])?(?:\s+.*)?$/i.test(text))
    return 96;
  if (
    /^(?:book|part|volume|vol\.|letter|stave|canto)\s+(?:the\s+)?[\divxlcdm]+(?:[.)])?(?:\s+.*)?$/i.test(
      text,
    )
  ) {
    return 90;
  ...
  if (
    /^(?:book|part|volume|vol\.|stave|canto)\s+(?:the\s+)?[a-z-]+(?:[.)])?(?:\s+.*)?$/i.test(
      text,
    )
  ) {
    return 88;
...
function cleanTitle(text: string): string {
  return normalizeText(embeddedStructuralHeading(text) ?? text)
    .replace(/\]+$/g, "")
    .slice(0, 220);
}
...
function paragraphsBetween(
  ...
  const slice = blocks.slice(
    start.blockIndex + 1,
    next?.blockIndex ?? blocks.length,
  );
  return slice
    .filter(isParagraphBlock)
    .map((block) => cleanParagraphText(block.text));
}

function filterStartsWithBody(
  blocks: HtmlBlock[],
  starts: ChapterStart[],
): ChapterStart[] {
  const structuralStartIndex = starts.findIndex(
    (start) => start.confidence >= 76,
  );
  const scopedStarts =
    structuralStartIndex > 0 &&
    starts.filter((start) => start.confidence >= 76).length >= MIN_HTML_CHAPTERS

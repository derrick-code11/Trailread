...
export async function assertPodcastGenerationRateLimit(userId: string): Promise<void> {
  ...
  const key = `rl:podcast-generate:${userId}:${dayBucket()}`
  const n = await redis.incr(key)
  if (n === 1) {
    await redis.expire(key, WINDOW_SEC)
  }
  if (n > LIMIT) {
    throw new AppError('RATE_LIMITED', 429, 'Too many podcast recap generations today. Try again tomorrow.', {})
  }
}
